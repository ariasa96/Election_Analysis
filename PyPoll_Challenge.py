...
with open(file_to_save, "w") as txt_file:
    ...
    for county in counties:
        ...
        county_vote = counties_dict.get(county)
        # 6c: Calculate the percentage of votes for the county.
        vote_percentage = float(county_vote) / float(total_votes) * 100

         # 6d: Print the county results to the terminal.
        results = f"{county}: {vote_percentage:.1f}% ({county_vote:,})\n"
         # 6e: Save the county votes to a text file.
        txt_file.write(results)
         # 6f: Write an if statement to determine the winning county and get its vote count.
        if (county_vote > largest_turnout_county_count) and (
            vote_percentage > largest_county_percentage):
